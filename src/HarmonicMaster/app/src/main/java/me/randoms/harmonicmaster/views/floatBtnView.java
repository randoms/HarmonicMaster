...
public class floatBtnView extends View{
	...
	protected void onDraw(Canvas canvas) {
        ...
        image = drawableToBitmap(icon);

        // init shader
        if (image != null) {
            int centerX = getWidth()/2;
            int centerY = getHeight()/2;
            int bitmapSize = (int) DisplayUtils.convertDpToPixel(24,getContext());
            Bitmap mBitmap = Bitmap.createScaledBitmap(image,
                    bitmapSize,
                    bitmapSize, false);
            canvas.drawBitmap(mBitmap,centerX-bitmapSize/2,centerY-bitmapSize/2,paint);
